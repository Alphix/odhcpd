...
static FILE *statefiles_open_tmp_file(int dirfd)
{
	...
	fd = openat(config.dhcp_hostsdir_fd, ODHCPD_TMP_FILE, O_CREAT | O_WRONLY | O_CLOEXEC, 0644);
	if (fd < 0)
		goto err;
...
void statefiles_read_prefix_information(struct interface *iface)
{
	...
	while ((line = fgets(buf, sizeof(buf), fp))) {
		...
		char *lifetime_str = strtok(NULL, "\n");
		long long length, lifetime;
		char *end;

		if (!prefix_str || !length_str || !lifetime_str)
			continue;

		if (inet_pton(AF_INET6, prefix_str, &pio.prefix) != 1)
			continue;

		length = strtoll(length_str, &end, 10);
		if (*end != '\0' || length < 1 || length > 128)
			continue;
		pio.length = length;

		lifetime = strtoll(lifetime_str, &end, 10);
		if (*end != '\0')
			continue;
